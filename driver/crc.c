...
//crc 校验算法
static const unsigned int g_crc32_tbl[256] =
{
...
#define _crc(accum, delta) 		(accum)=g_crc32_tbl[((accum)^(delta))&0xff]^((accum)>>8)
unsigned int CRC_CalCrc(char *pbuf, unsigned int size, unsigned int crc_val) //默认初始值crc_val=0xffffffff
{
	unsigned int temp			= 0;
	unsigned int j			= 0;
	for (; size; size -= 4)
...
/**************************************************
  ...
  *返回值    转换后的十六进制字符
   *************************************************/
static int BYTE2HEX(int  int_val)
{
    if (int_val>=0 && int_val<=9)
    {
        return  int_val + '0';
    }

    if (int_val>=0xA && int_val<=0xF)
     {
...
/////////////////////////////////////////////////////////////
static int HEX2BYTE(char  hex_ch)
{
